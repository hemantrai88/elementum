...
elementum.controller('elementumCtrl', ['$scope', function($scope) {
	...
	$scope.firstSelected = {};
	$scope.firstMatch = false;
	$scope.secondSelected = {};
	$scope.secondMatch = false;

	_.sortBy($scope.elements, 'atomic_number'); 

	$scope.firstList = [];
	$scope.firstListMatch = [];
	$scope.firstListIndex = 0;

	_.each($scope.elements, function(element, key) {
		if($scope.firstListIndex<=4){
			var temp = {key, element };
			var tempCopy = {key, element };
			temp.cardId = $scope.cardCount;
			$scope.cardCount++;
			tempCopy.cardId = $scope.cardCount;
			$scope.firstList.push(temp);
			$scope.firstList.push(tempCopy);
			$scope.firstListIndex++;
			shuffle($scope.firstList);
		}
	});

	$scope.launchGame = function() {
		console.log('START GAME', $scope.firstList);
		...
		_.delay(function() {
			...
			$('#startSplash').css('height', windowHeight);
		},0);
	};
	...
	$scope.matchElements = function() {
		if($scope.firstMatch && $scope.secondMatch){
			console.log('--------MATCHING----------');
			console.log('FIRST : ', $scope.firstSelected);
			console.log('SECOND : ', $scope.secondSelected);
			...
			_.delay(function() { $('.flip-me').removeClass('flip-me'); }, 2000);
